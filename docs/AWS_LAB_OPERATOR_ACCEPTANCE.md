...
| Release and infrastructure as code | The repaired pipeline passed tests, reviewed and applied saved Terraform plans, published an immutable version, verified it, promoted the alias, verified the service and produced a follow-up plan with no changes. |
...
Complete the separately recorded candidate and assessor browser checks, retain the immutable exercise/version evidence, and run calibrated engineer pilots before hiring use. Preserve the synthetic-only restriction until a separately reviewed assessment version is approved for that next stage.
