...
import { createKubernetesLabService, type LabServiceDependencies } from "../src/lib/recruit/kubernetes-lab-service";
...
function fixture(settings: LabServiceDependencies["settings"] = () => ({ url: "https://runner.invalid", key: "test-key" }), cleanupSettings?: LabServiceDependencies["cleanupSettings"]) {
  ...
  const db = {
    ...
    recruitmentLabSession: sessionTable,
    recruitmentLabCommand: commandTable,
  ...
  const service = createKubernetesLabService({ db: db as unknown as LabServiceDependencies["db"], request, settings, cleanupSettings });
  ...
  async function ready(id = "candidate-a", task = 1) {
    ...
    await service.startLab(id, task, KUBERNETES_LAB_TEMPLATE.id);
...
test("lab service rejects locked, unstarted and expired work before contacting the runner", async () => {
  for (const change of [{ status: "invited" }, { status: "submitted" }, { workLockedAt: new Date() }, { deadline: new Date(0) }]) {
    ...
    await assert.rejects(f.service.startLab("candidate-a", 1, KUBERNETES_LAB_TEMPLATE.id), (e: unknown) => e instanceof LabError && e.status === 403);
...
test("async settings gates are awaited before starting work", async () => {
  ...
  f.candidate();
  await assert.rejects(f.service.startLab("candidate-a", 1, KUBERNETES_LAB_TEMPLATE.id), LabError);
  assert.equal(f.calls.length, 0);
...
test("a stale start request cannot provision after submission wins the lock", async () => {
  ...
  await f.service.startLab("candidate-a", 1, KUBERNETES_LAB_TEMPLATE.id);
