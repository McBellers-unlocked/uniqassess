...
import { readLab, refreshLab, startLab, runLabCommand } from "@/lib/recruit/aws-lab-service";
...
async function authorise(token: unknown, number: unknown, mutating: boolean) {
  ...
  if (!task || !isMemoAiTask(task) || !task.awsLab) throw new LabError("This task does not have a practical lab.", 404);
  return { candidate: result.candidate, task, lab: task.awsLab };
}
