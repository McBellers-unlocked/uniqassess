      ...
      {session && (
        <div className="space-y-3 border-b border-uq-faint px-4 py-4">
          ...
          <p id={`lab-help-${taskNumber}`} className="text-xs leading-relaxed text-uq-3">{aws
            ? <>Run non-interactive commands with AWS CLI, Terraform and the supplied pipeline tools. Each job may run for up to five minutes, within the remaining assessment time. Jobs run in the background; this page polls their recorded status, so wait for the result before starting another job. Files and cloud changes persist during the lab; shell variables do not. Use a single run for commands that need to share variables.</>
