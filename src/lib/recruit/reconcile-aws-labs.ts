...
import { createAwsLabService } from "./aws-lab-service";
import { resolveRunnerCleanupSettings, runnerCleanupRequest } from "./aws-lab-runner";
  ...
  for (let offset = 0; offset < sessions.length; offset += 4) {
    ...
    const batch = await Promise.allSettled(sessions.slice(offset, offset + 4).map(async (session) => {
      try { await refreshLab(session.candidateId, session.taskNumber, true, { allowDispatch: false }); }
        ...
        await prisma.recruitmentAwsLabSession.update({ where: { id: session.id }, data: { error: "Scheduled lab reconciliation could not reach the runner. Retry required." } });
        throw error;
      }
    }));
