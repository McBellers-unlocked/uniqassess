import type { PrismaClient, RecruitmentLabSession, RecruitmentLabCommand } from "@prisma/client";
import {
...
} from "./kubernetes-lab-runner";

type LabDatabase = Pick<PrismaClient, "$transaction" | "recruitmentCandidate" | "recruitmentLabSession" | "recruitmentLabCommand" | "recruitmentActivityEvent">;
...
export function createKubernetesLabService({
  ...
  async function readLab(candidateId: string, taskNumber: number) {
    const session = await db.recruitmentLabSession.findUnique({
  ...
  async function persistCommand(sessionId: string, result: RunnerCommand) {
    ...
    await db.recruitmentLabCommand.updateMany({
  ...
  async function persistSession(session: RecruitmentLabSession, remote: RunnerSession) {
    if (remote.snapshot || remote.cleanupComplete) {
      await db.recruitmentLabSession.update({
        where: { id: session.id },
        data: {
    ...
    await db.recruitmentLabSession.updateMany({
      where: { id: session.id, status: { notIn: LAB_TERMINAL_STATUSES } },
      data: { status: remote.status },
    });
  }

  async function provision(session: RecruitmentLabSession): Promise<RunnerSession | null> {
    return db.$transaction(async (tx) => {
      ...
      const current = await tx.recruitmentLabSession.findUnique({ where: { candidateId_taskNumber: { candidateId: session.candidateId, taskNumber: session.taskNumber } } });
  ...
  async function dispatchCommand(session: RecruitmentLabSession, command: RecruitmentLabCommand): Promise<RunnerCommand | "closed" | "settled"> {
    return db.$transaction(async (tx) => {
      ...
      const saved = await tx.recruitmentLabCommand.findUnique({ where: { id: command.id } });
  ...
  async function syncCommands(session: RecruitmentLabSession, mayDispatch: boolean, workClosed = true) {
    const pending = await db.recruitmentLabCommand.findMany({
  ...
  async function startLab(candidateId: string, taskNumber: number, templateId: string) {
    ...
    const session = await db.$transaction(async (tx) => {
      ...
      const previous = await tx.recruitmentLabSession.findUnique({ where: { candidateId_taskNumber: { candidateId, taskNumber } } });
      if (previous) {
      ...
      const created = await tx.recruitmentLabSession.create({ data: { candidateId, taskNumber, templateId, expiresAt } });
      await tx.recruitmentActivityEvent.create({ data: { candidateId, taskNumber, eventType: "lab_started", metadata: { templateId } } });
      return created;
    });
  ...
  async function runLabCommand(candidateId: string, taskNumber: number, command: string, requestId: string) {
    ...
    const saved = await db.$transaction(async (tx) => {
      ...
      const candidate = await tx.recruitmentCandidate.findUnique({ where: { id: candidateId } });
      if (!candidate || !labWorkIsActive(candidate)) throw new LabError("Assessment work is locked or the time has expired.", 403);
      const session = await tx.recruitmentLabSession.findUnique({ where: { candidateId_taskNumber: { candidateId, taskNumber } } });
      ...
      const previous = await tx.recruitmentLabCommand.findUnique({ where: { sessionId_requestId: { sessionId: session.id, requestId } } });
      if (previous) {
      ...
      const pending = await tx.recruitmentLabCommand.count({ where: { sessionId: session.id, status: { in: LAB_ACTIVE_COMMAND_STATUSES } } });
      if (pending) throw new LabError("Wait for the current command to finish, then refresh the lab.", 409);
      if (await tx.recruitmentLabCommand.count({ where: { sessionId: session.id } }) >= LAB_MAX_COMMANDS) {
        throw new LabError(`This lab has reached its ${LAB_MAX_COMMANDS}-command limit.`, 429);
      }
      const created = await tx.recruitmentLabCommand.create({ data: { sessionId: session.id, requestId, command } });
  ...
  async function stopLab(session: RecruitmentLabSession, status: "stopped" | "expired" | "failed", includeCommandEvidence = true) {
    await db.recruitmentLabSession.updateMany({
  ...
  async function closeCandidateLabs(candidateId: string) {
    const sessions = await db.recruitmentLabSession.findMany({
  ...
  async function reconcileCandidateLabs(candidateId: string) {
    const sessions = await db.recruitmentLabSession.findMany({ where: { candidateId }, orderBy: { taskNumber: "asc" }, take: 2 });
...
type LabService = ReturnType<typeof createKubernetesLabService>;
let defaultService: Promise<LabService> | undefined;
function service() {
  return defaultService ??= import("@/lib/prisma").then(({ prisma }) => createKubernetesLabService({
