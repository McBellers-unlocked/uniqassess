...
Publish a known-good immutable version from the reference app (see `assessor/build_solution.py:reference_app`) and record its version number. Then deploy the candidate seed app as `$LATEST`; create alias `live` pointing to the known-good immutable version. Published versions use the same role, so the initially wrong inline policy will also prevent the known-good alias from reading orders until repaired. Restore/test the bounded correct policy while verifying the known-good version, then seed the deliberate incorrect policy before candidate access. Never label an unverified version known-good.
